...
public class CommandValidation(string command)
{
    ...
    public CommandValidation SplitIntoCommandAndArgs()
    {
        ...
        if (commandEnum == null) throw new ArgumentException($"'{commandAll.First().Trim()}' is not a valid command.");

        var args = commandAll.Length > 1 && !string.IsNullOrWhiteSpace(commandAll[1]) ? commandAll[1].Trim() : null;
